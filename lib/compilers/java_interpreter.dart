...
/// Comprehensive Java code interpreter for farm drone operations
/// Supports: variables, operators, if-else, switch-case, loops, break/continue, System.out.println, try-catch
class JavaInterpreter extends FarmCodeInterpreter {
  ...
  Future<void> _handlePrintln(String stmt) async {
    if (stmt.endsWith(';')) stmt = stmt.substring(0, stmt.length - 1);

    final match = RegExp(r'System\.out\.println\((.*?)\)$').firstMatch(stmt);
    if (match == null) {
      throw Exception('Syntactical Error: Invalid println statement');
    }
